...
class Select extends BaseComponent{
  ...
  onClick = (event) => {
    ...
    const isUotsideDropdownClick = 
      target.closest(this.selectors.dropdown) !== this.dropdownElement

    if (!isButtonClick && isUotsideDropdownClick) {
      this.collapse()
      return
    }
  ...
  onMobileMatchMediaChenge = (event) => {
    this.updateTabIndexes(event.matches)
  }

  onOriginalControlsChange = () => {
    this.state.selectedOptionElement = this.optionElements[this.originalControlElement.selectedIndex]
  }

  bindEvents() {
    MatchMedia.mobile.addEventListener('change', this.onMobileMatchMediaChenge)
    this.buttonElement.addEventListener('click', this.onButtonClick)
    document.addEventListener('click', this.onClick)
    this.rootElement.addEventListener('keydown', this.onKeydown)
    this.originalControlElement.addEventListener('change', this.onOriginalControlsChange)
  }
}
